/**
  ...
  *
  * TP02 - Questao 08 - Pesquisa Binaria em C
  *  
...
int main( int argc, char** argv){
   ...
   char chaves[NUMENTRADA][TAMLINHA];
   int nChaves = 0;

   do{

      fgets(chaves[nChaves], TAMLINHA, stdin);
      chaves[nChaves][strcspn(chaves[nChaves], "\n")] = '\0'; // elimina '\n'

   }while(isFim(chaves[nChaves++]) == false);
   ...
   for (int i = 1 ; i < nChaves ; i++){
      ...
      t1 = clock();
      resposta = pesquisaBinaria(chaves[i]);
      t2 = clock();
      ...
      char log[100];
      snprintf(log, 100, "635383\t %6fms\t Comp: %d\n", tempo, comparacoes);
